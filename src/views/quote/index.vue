...
import { selectQuoteCount, selectQuoteList } from "@/api/meta";
export default {
  mixins: [mixin],
  data() {
    return {};
  ...
  methods: {
    handleQuery() {
      Promise.all([selectQuoteList({ type: "option", slug: "default", size: Number.MAX_SAFE_INTEGER }), selectQuoteCount({ columns: ["type"] })]).then((res) => {
