<template>
  <div>
    <el-form ref="form" :model="form" :inline="true" size="small">
      ...
      <el-form-item>
        <el-select v-model="form.keywords" placeholder="关键字" clearable filterable remote allow-create :remote-method="getKeywordList" @change="keywordOptions = []">
  ...
  methods: {
    ...
    handleSubmitDialog() {
      if (this.dialog.target === "insert") {
        insert_script(this.dialog.form).then((res) => {
      ...
      if (this.dialog.target === "update") {
        update_script(this.dialog.form).then((res) => {
    ...
    getKeywordList(str) {
      ...
      select_script_keywords(str).then((res) => {
